...
class StudentManagementSystem:
    def __init__(self, root):
        self.root = root  # 指定这个菜单属于哪个父窗口（主窗口）
        self.root.title("学生信息管理系统")   #设置窗口标题
        self.root.geometry("900x600")        #设置窗口大小
    ...
    def create_database(self):
        """创建数据库和表"""
        conn = sqlite3.connect('students.db') #连接SQLite数据库文件"students.db"
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS students
                     (id INTEGER PRIMARY KEY AUTOINCREMENT,
                      ...
                      age INTEGER,
                      major TEXT,
                      email TEXT)''')  #执行SQL语句创建数据表
        conn.commit()  # 提交事务，使创建表的操作永久生效
        conn.close()   # 关闭数据库连接
    
    def create_menu(self):
        """创建菜单栏"""
        menubar = Menu(self.root) #创建一个菜单对象
        
        # 文件菜单
        file_menu = Menu(menubar, tearoff=0) #tearoff=0：禁用菜单的可分离特性（设为1则允许用户"撕下"菜单成为独立窗口）
